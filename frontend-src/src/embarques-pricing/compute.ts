// Cálculo de costo landed en el cliente, SOLO para vista previa instantánea
// mientras el usuario edita. La fuente de verdad es el backend
// (backend/embarques_pricing/service.py); esta función ESPEJA esa lógica para
// que el preview coincida 1:1 con lo que se guarda. Mantener ambas en sync.
...
export function calcularLanded(
  ...
  const rows: LandedRow[] = items.map((it) => {
    ...
    const pesoU = Number(it.peso_unit_lbs) || 0
    ...
    return {
      ...it,
      cantidad: cant, peso_unit_lbs: pesoU, fob_unit: fobU, tc_valor: tc,
      peso_total_lbs: pesoTotal, fob_total: fobTotal, fob_clp: fobTotal * tc,
      shipping_clp: 0, cif_clp: 0, gastos_clp: 0, costo_total_clp: 0, costo_unit_clp: 0,
    }
  })

  const sumaPeso = rows.reduce((s, r) => s + r.peso_total_lbs, 0)
  prorratear(rows, ship, sumaPeso > 0 ? 'peso_total_lbs' : 'fob_clp', 'shipping_clp')
  ...
  const totales: LandedTotals = {
    peso_total_lbs: rows.reduce((s, r) => s + r.peso_total_lbs, 0),
