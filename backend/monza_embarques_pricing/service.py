"""Cálculo de costo landed por ítem de un embarque (MonzaParts).

Réplica EXACTA de la metodología de Grupo AM (backend/embarques_pricing/service.py).
Función pura: no toca la base de datos. La única diferencia es de nomenclatura: el peso
se maneja en kg (clave `peso_unit`) en vez de libras; la matemática de prorrateo es
idéntica (es proporcional, la unidad no altera el resultado).
...
`total_gastos` capitaliza SOLO los netos de Desconsolidación, Almacenaje, Agencia de
Aduana, Arancel/Derechos y Otros. EXCLUYE el IVA y el IVA Importación (recuperables).

Robustez (no rompen el cuadre):
  · Si Σ pesos = 0, el shipping se prorratea por FOB CLP; si tampoco hay FOB, partes
    iguales (el shipping nunca se "pierde").
  · Si Σ CIF = 0, los gastos se reparten en partes iguales.
  · Σ prorrateos = 1 → Σ shipping_i = shipping_total y Σ gastos_i = total_gastos.
"""
...
def _prorratear(rows: List[Dict], total: float, base_key: str, out_key: str) -> None:
    """Reparte `total` entre `rows` proporcional a `base_key`. Fallback: partes iguales."""
    n = len(rows)
    if n == 0:
        return
...
def calcular_landed(
    ...
    Args:
        items: dicts con al menos: cantidad, peso_unit, fob_unit, tc_valor.
        shipping_total_clp: flete total del embarque, en CLP.
        total_gastos_capitaliza: suma de gastos locales que capitalizan (netos), CLP.
    ...
    suma_peso = sum(r["peso_total"] for r in rows)
    base_ship = "peso_total" if suma_peso > 0 else "fob_clp"
    ...
    for r in rows:
        ...
        r["costo_unit_clp"] = r["costo_total_clp"] / r["cantidad"] if r["cantidad"] else 0.0

    totales = {
        "n_items": len(rows),
        "peso_total": sum(r["peso_total"] for r in rows),
...
# ─── Catálogo de gastos locales (mismo del módulo de Grupo AM) ────────────────
