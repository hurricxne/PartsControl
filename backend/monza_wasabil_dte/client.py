"""Cliente HTTP del API REST de Wasabil (facturador electrónico de MonzaParts).
...
- Base y token vienen de config.py (backend/.env): WASABIL_API_BASE / WASABIL_API_TOKEN_MONZA.
...
class WasabilNoConfigurado(WasabilError):
    """No hay WASABIL_API_TOKEN_MONZA en backend/.env — no se puede llamar a Wasabil."""
...
def _request(metodo: str, path: str, json_body: Optional[dict] = None,
    ...
    if resp.status_code == 401:
        raise WasabilError("Wasabil rechazó el token (401). Revisa WASABIL_API_TOKEN_MONZA "
