...
# DELIBERADA de monza_wasabil_compras (_folio_blando / rut_canonico): los helpers
# no se comparten entre paquetes (regla de la casa). Espejo de compras_contab (GA).
...
def _rut_canon_dup(x):
    ...
    escrito con puntos y un ESPACIO en lugar del guión. Pero
    `monza_cont_compra.proveedor_rut` es TEXTO LIBRE tecleado a mano: el dato sucio es
    el caso normal, no el raro. Con la compra vieja guardada como '96.631.520 2',
    `_rut_canon_dup` devolvía None sobre la CANDIDATA, la fila se descartaba del bucle
    y el guard quedaba apagado justo para ella — mientras el guard exacto tampoco choca
    ('96631520-2' ≠ '96.631.520 2') y el UNIQUE (proveedor_rut,
    numero_documento_activo) compara el TEXTO del RUT. Resultado: dos CxP ACTIVAS por
    la misma factura física, las dos pagables por Tesorería. O sea, el doble pago que
    este guard existe para impedir.

    El propio paquete del libro SII ya había resuelto esto y validado la regla
    (`_rut_canon_sucio` en monza_wasabil_compras/matcher.py): si la forma estricta se
    rinde y el texto tiene un espacio, se reintenta cambiando el espacio por el guión.
    Se copia rama por rama (los helpers no se comparten entre paquetes: regla de la
    casa). Gemelo del arreglo de compras_contab (GA), escrito por separado.
...
def _crear_compra_tx(payload: CompraCreate, db: Session, current_user: User):
    ...
    if payload.numero_documento:
        ...
        if rut_obj and folio_obj:
            candidatas = (db.query(MonzaContCompra.id, MonzaContCompra.proveedor_rut,
                                   MonzaContCompra.numero_documento,
                                   MonzaContCompra.tipo_doc)
                          .filter(MonzaContCompra.anulado.is_(False),
                                  MonzaContCompra.numero_documento.isnot(None)).all())
