"""Cliente REST del libro de compras — SOLO LECTURA contra el API real de Wasabil.
...
TOKEN: el MISMO de la emisión de Grupo AM (WASABIL_API_TOKEN de backend/.env, ya declarado
en config.py — cero variables nuevas, cero riesgo de extra_forbidden). Sin token, el módulo
se declara no configurado y el barrido no corre: jamás un guard que "funciona" sin poder ver.
"""
import logging
