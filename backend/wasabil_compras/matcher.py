"""El MOTOR del matcher banco ↔ libro SII ↔ egresos — Grupo AM / MachParts.

Implementa la spec docs/spec-matcher-banco-libro-2026-08-06.md. PRINCIPIO RECTOR:
un match falso AUTOMÁTICO es peor que cien sugerencias — AUTO es una CONJUNCIÓN de
condiciones duras con identidad de DOCUMENTO; el score numérico SOLO ordena sugeridos.
Ante la duda: degradar de AUTO a SUGERIDO, y de SUGERIDO a nada.
...
pierde la noche) y JAMÁS toma FOR UPDATE sobre sii_libro_doc — el tope por doc se
serializa con el lock del MOVIMIENTO + re-lectura fresca, y la coherencia de contenido
la da el snapshot de hash. Idempotente: N corridas = 1 corrida; jamás revive
...
Imports de tesoreria/compras_contab LOCALES dentro de funciones (mismo patrón que
_llaves_erp del router hermano): este paquete no se acopla a esos módulos al arranque.
Aritmética de comparación SIEMPRE en pesos ENTEROS (round) — regla 8 de la spec.
"""
import json
import logging
...
from .models import (
    SiiLibroDoc, SiiLibroMatch, SiiLibroReglaRut, SiiMatchConfig,
    SiiMatchEtiquetaMov, SiiMatchRun,
)
...
MEDIO_ADELANTO = "adelanto"     # mismo literal que routers/contabilidad (no se importa
                                # ese módulo: arrastra el árbol completo de la app)

# ── Configuración mantenible sin redeploy (tabla sii_match_config pisa esto) ────
CONFIG_DEFAULTS = {
...
def _clave_mov_de(mov) -> list:
    ...
    (tesoreria/router.py:210-218): con ella los descartes sobreviven al re-import de
    la cartola (regla 5)."""
...
def _con_retry_deadlock(db: Session, operacion):
    ...
    tesoreria/router.py:124-153: se reintenta la transacción COMPLETA tras rollback y
    a la tercera se desiste (acá devolviendo el error al caller, que degrada el AUTO
    a sugerido: perder un auto es barato, perder la corrida no)."""
...
def cargar_config(db: Session) -> dict:
    """Defaults del código pisados por las filas de sii_match_config (JSON por clave):
    cambiar la lista de factors o encender rut_en_glosa NO exige deploy."""
    cfg = {k: (dict(v) if isinstance(v, dict) else list(v) if isinstance(v, list) else v)
           for k, v in CONFIG_DEFAULTS.items()}
    for fila in db.query(SiiMatchConfig).all():
        try:
            cfg[fila.clave] = json.loads(fila.valor)
        except (ValueError, TypeError):
            logger.warning("sii_match_config['%s']: JSON inválido, se ignora", fila.clave)
    return cfg


def _campos_doc(doc: SiiLibroDoc) -> dict:
...
def docs_no_bancarios(db: Session, llaves: Optional[Dict[tuple, list]] = None) -> set:
    ...
    if not pagadas:
        return set()
    medios = (db.query(ContEgresoDetalle.compra_id, ContEgreso.medio)
              .join(ContEgreso, ContEgreso.id == ContEgresoDetalle.egreso_id)
              .filter(ContEgresoDetalle.compra_id.in_(list(pagadas.values())))
              .all())
...
class _Contexto:
    ...
    def __init__(self):
        ...
        self.docs_pool: Dict[int, SiiLibroDoc] = {}
        self.saldo_doc: Dict[int, int] = {}
        self.docs_cubiertos: Dict[int, SiiLibroDoc] = {}   # saldo 0 → N2 doble pago
        self.no_bancario_ids: set = set()
        self.residual_mov: Dict[int, int] = {}
        self.by_par: Dict[tuple, SiiLibroMatch] = {}
        self.matches: List[SiiLibroMatch] = []
        ...
        self.reglas_rut: Dict[str, SiiLibroReglaRut] = {}
...
def _construir_contexto(db: Session, cfg: dict) -> _Contexto:
    from compras_contab.models import ContCompra, ContEgreso, ContEgresoDetalle
    from tesoreria.models import (Conciliacion, ConciliacionIngreso, CuentaBancaria,
                                  MovimientoBancario)

    ctx = _Contexto()
    ctx.cfg = cfg
    ctx.reglas_rut = {r.rut_canonico: r for r in db.query(SiiLibroReglaRut).all()}

    # ── Matches existentes: el estado previo manda sobre la corrida ──────────────
    ctx.matches = db.query(SiiLibroMatch).all()
    ...
    docs = (db.query(SiiLibroDoc)
            .filter(SiiLibroDoc.estado_espejo == "ACTIVO",
                    SiiLibroDoc.monto_efectivo.isnot(None)).all())
    hoy = date.today()
    ...
    #    tesoreria/router.py:221-226 — en otra moneda los montos no son comparables),
    #    etiquetados los sin-contraparte y con residual sobre CONFIRMADOS ───────────
    movs = (db.query(MovimientoBancario)
            .join(CuentaBancaria, CuentaBancaria.id == MovimientoBancario.cuenta_id)
            .filter(MovimientoBancario.empresa == "mineria",
                    MovimientoBancario.tipo.in_(["cargo", "abono"]))
            .all())
    movs = [m for m in movs
            if (getattr(m.cuenta, "moneda", None) or "CLP") == "CLP"]
    etiquetas = {e.movimiento_id: e for e in db.query(SiiMatchEtiquetaMov).all()}
    nuevos = 0
    for m in movs:
    ...
    if mov_ids:
        concs = (db.query(Conciliacion)
                 .filter(Conciliacion.movimiento_id.in_(mov_ids)).all())
        egreso_ids = list({c.egreso_id for c in concs})
        egresos = {e.id: e for e in db.query(ContEgreso)
                   .filter(ContEgreso.id.in_(egreso_ids)).all()} if egreso_ids else {}
        ...
        if egreso_ids:
            for det, compra in (db.query(ContEgresoDetalle, ContCompra)
                                ...
                                .filter(ContEgresoDetalle.egreso_id.in_(egreso_ids)).all()):
        ...
        ctx.ingreso_movs = {r[0] for r in db.query(ConciliacionIngreso.movimiento_id)
                            .filter(ConciliacionIngreso.movimiento_id.in_(mov_ids)).all()}
    ...
    egresos_op = (db.query(ContEgreso)
                  .filter(ContEgreso.empresa == "mineria",
                          ContEgreso.numero_operacion.isnot(None)).all())
    for e in egresos_op:
    ...
    #    aprobados sin conciliar — mismas queries que el router de Tesorería ────────
    from models.models import ContAdelanto, ContCobranza, ContFacturaCliente
    pares = (db.query(ContCobranza.id, ContCobranza.monto)
             .join(ContFacturaCliente, ContFacturaCliente.id == ContCobranza.factura_id)
             .filter(ContFacturaCliente.empresa == "mineria",
                     ContCobranza.medio != MEDIO_ADELANTO).all())
    ya = {r[0] for r in db.query(ConciliacionIngreso.cobranza_id)
          .filter(ConciliacionIngreso.cobranza_id.isnot(None)).all()}
    ctx.montos_cobranza_adelanto = [_p(m) for cid, m in pares if cid not in ya]
    adels = (db.query(ContAdelanto.id, ContAdelanto.monto)
             .filter(ContAdelanto.empresa == "mineria",
                     ContAdelanto.estado == "aprobado").all())
    ya_a = {r[0] for r in db.query(ConciliacionIngreso.adelanto_id)
            .filter(ConciliacionIngreso.adelanto_id.isnot(None)).all()}
...
def _lotes_de_egresos(db: Session, ctx: _Contexto, movs_sin_candidato: list) -> int:
    ...
    from compras_contab.models import ContEgreso
    creadas = 0
    for mov in movs_sin_candidato:
        ...
        egresos = (db.query(ContEgreso)
                   .filter(ContEgreso.empresa == "mineria",
                           ContEgreso.conciliado.is_(False),
                           ContEgreso.fecha.in_(fechas))
                   .order_by(ContEgreso.id).limit(LOTE_MAX_EGRESOS_DIA * 2).all())
        ...
        et = (db.query(SiiMatchEtiquetaMov)
              .filter(SiiMatchEtiquetaMov.movimiento_id == mov.id).first())
        if et is None:
            db.add(SiiMatchEtiquetaMov(movimiento_id=mov.id,
                                       etiqueta="lote_pendiente", detalle=detalle))
            creadas += 1
...
def _insertar_auto(db: Session, p: dict) -> bool:
    ...
    conc_movimiento (calca _mov_scoped lock=True, tesoreria/router.py:108-121) con
    REVALIDACIÓN COMPLETA adentro (saldos frescos, conciliación fresca, Σ topes,
    unicidad), envuelto en retry de deadlock. JAMÁS FOR UPDATE sobre sii_libro_doc:
    el tope por doc se serializa con el lock del mov + re-lectura fresca."""
    from tesoreria.models import MovimientoBancario

    def _tx():
        mov = (db.query(MovimientoBancario)
               .filter(MovimientoBancario.id == p["mov"].id,
                       MovimientoBancario.empresa == "mineria")
               .populate_existing().with_for_update().first())
        if mov is None:
            db.rollback()
            return False
        doc = (db.query(SiiLibroDoc).filter(SiiLibroDoc.id == p["doc"].id)
        ...
        # sobre las filas de sii_libro_match del doc (índice ix_sii_match_doc_estado)
        ...
        (db.query(SiiLibroMatch.id)
         .filter(SiiLibroMatch.doc_id == p["doc"].id)
         .with_for_update().all())
        # Σ topes FRESCOS (invariantes de la regla 1, verificadas bajo el lock):
        frescos = (db.query(SiiLibroMatch)
                   .filter(SiiLibroMatch.estado == "confirmado",
                           (SiiLibroMatch.movimiento_id == mov.id)
                           | (SiiLibroMatch.doc_id == doc.id)).all())
        ...
        db.add(SiiLibroMatch(
            empresa="mineria", movimiento_id=mov.id, doc_id=doc.id,
    ...
    try:
        ...
        db.rollback()
        logger.exception("matcher: auto (mov %s, doc %s) abortado",
                         p["mov"].id, p["doc"].id)
        return False
...
def _liberar_conciliado_si_es_del_match(db: Session, mov_id: int,
    ...
    (conc_conciliacion / _ingreso) y sin OTRO match confirmado vivo. La conciliación
    interna de Tesorería jamás se toca desde acá."""
    from tesoreria.models import Conciliacion, ConciliacionIngreso, MovimientoBancario
    ...
    if mov is None or not mov.conciliado:
        return
    tiene_enlace = (db.query(Conciliacion.id)
                    .filter(Conciliacion.movimiento_id == mov_id).first() is not None
                    or db.query(ConciliacionIngreso.id)
                    .filter(ConciliacionIngreso.movimiento_id == mov_id).first()
                    is not None)
    otro = (db.query(SiiLibroMatch.id)
            .filter(SiiLibroMatch.movimiento_id == mov_id,
                    SiiLibroMatch.estado == "confirmado",
                    SiiLibroMatch.id != (salvo_match_id or -1)).first())
    if not tiene_enlace and otro is None:
        mov.conciliado = False
        mov.conciliado_at = None


def _motivo_append(m: SiiLibroMatch, texto: str):
    try:
...
def _reanclar_descartados(db: Session, ctx: _Contexto) -> int:
    ...
    for m in sueltos:
        try:
            ...
            continue
        candidatos = (db.query(MovimientoBancario)
                      .filter(MovimientoBancario.empresa == "mineria",
                              MovimientoBancario.tipo == clave[1]).all())
...
def _reverificar_confirmados(db: Session, ctx: _Contexto) -> dict:
    ...
    if ids:
        docs_all = {d.id: d for d in db.query(SiiLibroDoc)
                    .filter(SiiLibroDoc.id.in_(ids)).all()}
...
def _caducar_zombis(db: Session, ctx: _Contexto) -> int:
    ...
    docs_all = {d.id: d for d in db.query(SiiLibroDoc)
                .filter(SiiLibroDoc.id.in_(ids)).all()} if ids else {}
...
def _reevaluar_conflictos(db: Session, ctx: _Contexto) -> int:
    ...
    complementa la máquina de estados documentada en models.py."""
    n = 0
    conflictos = [m for m in ctx.matches if m.estado == "conflicto"]
    if not conflictos:
        return 0
    docs_all = {d.id: d for d in db.query(SiiLibroDoc)
                .filter(SiiLibroDoc.id.in_([m.doc_id for m in conflictos])).all()}
    for m in conflictos:
        doc = docs_all.get(m.doc_id)
        if m.movimiento_id is None:
            m.estado = "en_duda"
...
def _escribir_propuestas(db: Session, ctx: _Contexto, propuestas: list) -> dict:
    ...
    def _fila_de(p, estado):
        return SiiLibroMatch(
            empresa="mineria", movimiento_id=p["mov"].id, doc_id=p["doc"].id,
...
def _matcher_en_curso(db: Session, *, anteriores_a: Optional[int] = None) -> bool:
    ...
    q = (db.query(SiiMatchRun)
         .filter(SiiMatchRun.terminado_at.is_(None),
                 SiiMatchRun.iniciado_at > limite))
    if anteriores_a is not None:
        q = q.filter(SiiMatchRun.id < anteriores_a)
    return q.first() is not None


def correr_matcher(db: Session, *, origen: str = "manual",
                   usuario_id: Optional[int] = None) -> SiiMatchRun:
    ...
    run = SiiMatchRun(origen=origen, usuario_id=usuario_id)
    db.add(run)
    ...
    try:
        ...
        db.commit()
        logger.info("Matcher (%s) OK: %s sugeridos, %s autos, %s conflictos, "
        ...
        db.commit()
        logger.exception("Matcher (%s): corrida abortada", origen)
    return run
