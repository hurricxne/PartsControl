"""Tablas del espejo del libro de compras del SII — Grupo AM / MachParts.

Tres tablas, tres responsabilidades:
  · sii_libro_doc      — el ESPEJO: una fila por documento recibido, llave dura el uuid.
  · sii_libro_sync_run — la BITÁCORA de cada barrido (sin ella, "¿de cuándo son estos
                         datos?" no tiene respuesta y el tablero no puede decir su edad).
  · sii_libro_regla_rut— las REGLAS por emisor (BLOQUEADO / IGNORAR_AUTO / LOGISTICO) que
                         alimentan los defaults de la bandeja (Fase A2).

Las crea el create_all del arranque (tablas nuevas; no altera existentes). Ningún dato de
estas tablas participa de un cálculo de plata del ERP: son lectura, cruce y decisión.
"""
import hashlib
import json
...
class SiiLibroDoc(Base):
    """Un documento RECIBIDO del libro, tal como Wasabil lo declara.
    ...
    REGLAS DE VIDA (del plan, sección modelo de datos):
      ...
      · Si el contenido cambia DESPUÉS de una decisión (hash distinto al de decidir),
        ...
        (Regla 7).
    """
    __tablename__ = "sii_libro_doc"
    __table_args__ = (
        UniqueConstraint("uuid", name="uq_sii_libro_doc_uuid"),
        Index("ix_sii_libro_negocio", "rut_emisor_canonico", "sii_document_type_id",
              "folio"),
        Index("ix_sii_libro_fecha", "document_date"),
        {"mysql_engine": "InnoDB"},
    )
    ...
    ultimo_run_id = Column(Integer, ForeignKey("sii_libro_sync_run.id",
...
class SiiLibroSyncRun(Base):
    ...
    __tablename__ = "sii_libro_sync_run"
...
class SiiLibroReglaRut(Base):
    ...
    """
    __tablename__ = "sii_libro_regla_rut"
    __table_args__ = (
        UniqueConstraint("rut_canonico", name="uq_sii_libro_regla_rut"),
        {"mysql_engine": "InnoDB"},
    )
...
# ═══════════════════════════════════════════════════════════════════════════════════
# MATCHER banco ↔ libro SII ↔ egresos (spec docs/spec-matcher-banco-libro-2026-08-06.md)
# ═══════════════════════════════════════════════════════════════════════════════════
# CIERRE DEL GRAFO DE FKs (hallazgo confirmado 2026-08-06, paridad inversa del #13 de
# Monza): SiiLibroMatch apunta a conc_movimiento (tesoreria) y ese grafo sigue con
# conc_conciliacion → cont_egreso (compras_contab) → emb_pricing_gasto
# (embarques_pricing). Sin estos imports, quien importe ESTE módulo de forma AISLADA
# (un test del paquete, un script, o un create_all sobre BD fresca que solo haya
# cargado los imports incondicionales de main.py) muere con NoReferencedTableError
# según el orden de imports — la misma lección que monza_wasabil_compras/models.py
# documenta del hallazgo F8. La sonda §11 de tests/test_a1_a2.py lo verifica en un
# intérprete limpio.
import tesoreria.models          # noqa: E402,F401  (registra conc_movimiento y conc_*)
import compras_contab.models     # noqa: E402,F401  (registra cont_egreso, FK de conc_conciliacion)
import embarques_pricing.models  # noqa: E402,F401  (registra emb_pricing_gasto, FK de cont_compra)


class SiiLibroMatch(Base):
    """La TABLA PUENTE: un enlace (movimiento bancario, documento del libro).
    ...
    FK a conc_movimiento con ondelete=SET NULL y movimiento_id NULLABLE — desviación
    RAZONADA del RESTRICT del DDL conceptual, porque las DOS sondas de la Regla 5 solo
    son satisfacibles a la vez sin tocar tesoreria/router.py (fuera del alcance):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("movimiento_id", "doc_id", name="uq_sii_match_par"),
        Index("ix_sii_match_doc_estado", "doc_id", "estado"),      # tope Σ por doc
        Index("ix_sii_match_mov_estado", "movimiento_id", "estado"),  # tope Σ por mov
        Index("ix_sii_match_estado", "estado"),                    # bandeja
        Index("ix_sii_match_grupo", "grupo_uuid"),
        {"mysql_engine": "InnoDB"},
    )

    id = Column(Integer, primary_key=True)
    empresa = Column(String(50), nullable=False, server_default="mineria")
    movimiento_id = Column(Integer, ForeignKey("conc_movimiento.id",
                                               ondelete="SET NULL"), nullable=True)
    doc_id = Column(Integer, ForeignKey("sii_libro_doc.id", ondelete="RESTRICT"),
                    nullable=False)
    ...
    # Los 6 campos de _clave_mov (tesoreria/router.py): los descartes sobreviven al
    # re-import de cartola re-anclándose por esta clave suave.
...
class SiiMatchRun(Base):
    """Bitácora de UNA corrida del matcher (espejo de sii_libro_sync_run): sin ella,
    «¿de cuándo son estas sugerencias?» no tiene respuesta."""
    __tablename__ = "sii_match_run"
    __table_args__ = ({"mysql_engine": "InnoDB"},)
...
class SiiMatchEtiquetaMov(Base):
    ...
    en un router (tesoreria) que este módulo no puede tocar.
    """
    __tablename__ = "sii_match_etiqueta_mov"
    __table_args__ = (
        UniqueConstraint("movimiento_id", name="uq_sii_etiq_mov"),
        {"mysql_engine": "InnoDB"},
    )
...
class SiiMatchConfig(Base):
    ...
    __tablename__ = "sii_match_config"
    __table_args__ = (
        UniqueConstraint("clave", name="uq_sii_match_config_clave"),
        {"mysql_engine": "InnoDB"},
    )
