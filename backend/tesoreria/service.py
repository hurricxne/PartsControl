"""Lógica pura del módulo Tesorería (autocontenida, sin FastAPI ni BD).

Incluye el parser FLEXIBLE de cartolas (CSV / Excel .xlsx) que detecta las columnas
por nombre de encabezado (fecha, glosa/detalle, cargo/abono o monto, referencia,
saldo) y normaliza montos en formato chileno ("1.234.567" / "1.234.567,89").
Helpers (_f, _parse_date, empresa_de) copiados del patrón del repo para mantener el
...
TIPOS_MOV = ["cargo", "abono"]   # cargo = egreso (pagos); abono = ingreso (cobranzas)
BANCOS_SUGERIDOS = [
    "Santander", "Banco de Chile", "BCI", "BancoEstado", "Itaú", "Scotiabank",
    "Security", "BICE", "Banco Falabella", "Banco Ripley", "Coopeuch", "Otro",
]

# Buckets del flujo de caja proyectado (días respecto de hoy) — NIC 7.
...
def parse_monto_cl(s) -> Optional[float]:
    ...
    # OJO: los dos replace de "espacio" de abajo NO son redundantes: el primero es espacio
    # ASCII (0x20) y el segundo es espacio duro / non-breaking space (0xA0), común como
...
def serialize_movimiento(m, *, destino: Optional[dict] = None) -> dict:
    """`destino` es el resumen de lo conciliado: un egreso de Compras (clase='egreso')
    ...
    return {
        ...
        "destino": destino,  # resumen del egreso/cobranza conciliado (si aplica)
    }
