...
def _correr_matcher_post_cartola(usuario_id: Optional[int]) -> None:
    ...
    razón. El operador pagaba esa espera en el spinner y, a los 60 s, el axios del
    frontend cortaba y le mostraba «no se pudo importar la cartola» con la cartola YA
    importada. Peor: el endpoint es `async def` con Session SÍNCRONA, así que la corrida
    bloqueaba el EVENT LOOP — no demoraba solo al que importaba, congelaba todas las
    peticiones del proceso. Y el costo crece para siempre: la tabla de movimientos solo
    engorda.
    ...
    PROPIA (molde `scheduler._matcher_post_barrido_ga`): la del request ya está cerrada
    cuando esto corre. Todo atrapado adentro: un fallo del motor JAMÁS toca una
    importación ya commiteada, y queda en el log y en la bitácora de corridas que el
    tablero del Libro SII muestra.
    """
    try:
        from database import SessionLocal
        from wasabil_compras.matcher import correr_matcher
        db = SessionLocal()
        try:
...
def _match_libro_confirmado(db, mov_ids):
    ...
    CONFIRMADO vivo de esos movimientos, o None.
    ...
    if not mov_ids:
        return None
    try:
        from wasabil_compras.models import SiiLibroMatch
    except ImportError:
        ...
        try:
            presente = importlib.util.find_spec("wasabil_compras") is not None
    ...
    try:
        fila = (db.query(SiiLibroMatch.id)
                .filter(SiiLibroMatch.movimiento_id.in_(list(mov_ids)),
                        SiiLibroMatch.estado == "confirmado").first())
...
def eliminar_cartola(cartola_id: int, db: Session = Depends(get_db),
    ...
    mov_ids_cartola = [r[0] for r in db.query(MovimientoBancario.id)
                       .filter(MovimientoBancario.cartola_id == cartola.id).all()]
