"""Migración: RUT canónico del proveedor (Fase 0 del libro de compras del SII).

QUÉ AGREGA
    proveedores.rut  VARCHAR(20) NULL  + índice ix_proveedores_rut — Grupo AM / MachParts.

POR QUÉ
    El ERP no guardaba el RUT del proveedor en ninguna parte (verificado: la tabla
    `proveedores` no tenía la columna; el único RUT era cont_compra.proveedor_rut, texto
    libre a mano). El libro de compras del SII identifica a cada emisor por RUT: sin una
    columna canónica no hay cruce posible, y el informe de faltantes mentiría el día uno.
    Forma canónica: '76513680-6' (wasabil_compras/rut.py).
...
Uso (desde backend/, con el venv activo):
    python -m migrations.proveedor_rut
"""
    ...
    with engine.begin() as conn:
        ...
        if not existe_tabla:
            raise SystemExit(
                ...
                "de Grupo AM/MachParts; revisa contra qué base estás corriendo")
        ...
        if _existe_indice(conn):
            ...
            # por documento del libro contra proveedores). La tabla es chica: el ALTER no
            # bloquea de forma apreciable.
