"""API del libro de compras del SII Monza — tablero, barrido, bandeja y decisiones (A1+A2).

Prefijo /api/monza/sii-libro. SOLO MonzaParts (candado 'automotriz' a nivel de router —
los datos del libro son de la empresa que emite con el RUT de LOPEZ HERNANDEZ INVERSIONES
SPA, 78.121.316-0; mismo patrón que monza_wasabil_dte/router.py). Sin gate de feature: es
lectura y decisión, no toca plata del ERP — el router se monta SIEMPRE.

ADAPTACIONES Monza vs GA (wasabil_compras/router.py, el molde):
  · El cruce ERP va contra MonzaContCompra (monza_compras_contab.models) — OJO: la clase
    NO se llama ContCompra como en GA, y NO tiene columna `empresa` (la separación entre
    marcas en Monza es POR TABLA): se filtra SOLO por `anulado`.
  · El proveedor es MonzaProveedor (monza_models) — su columna `rut` la agrega la
    migración migrations/monza_proveedor_rut.py.
  · Los centros de costos sugeridos son los MISMOS que GA (decisión del encargo).
...
from monza_models import MonzaProveedor
...
from .models import MonzaSiiLibroDoc, MonzaSiiLibroReglaRut, MonzaSiiLibroSyncRun
...
router = APIRouter(
    prefix="/api/monza/sii-libro",
    tags=["monza-sii-libro"],
...
# Tercera salida (hallazgo ALTO del controller): una factura grande de un solo documento
# no es costo de mercadería NI gasto del período — es un ACTIVO FIJO que se deprecia.
# Sin esta salida, el operador la forzaría a una de las dos y distorsionaría el P&L o
# el landed.
DESTINO_ACTIVO_FIJO = "activo_fijo"
# Los MISMOS centros sugeridos que GA (decisión del encargo del port).
CENTROS_SUGERIDOS = [
...
# Familia del tipo de DTE del SII, para poder compararla contra
# `monza_cont_compra.tipo_doc` (hallazgo #11, revisión 2026-08-08). Las series de folio
# del SII son independientes POR TIPO y por emisor: el mismo proveedor puede mandarnos la
# factura 33 N° 12 y la boleta 39 N° 12, que son dos documentos distintos. Sin el tipo en
# la llave del cruce, la boleta se declaraba «ESTA» contra la factura ya registrada,
# salía del filtro de faltantes y del CSV, y esa CxP no nacía nunca.
#
...
def _llaves_erp(db: Session):
    ...
    ACTIVAS de MonzaParts — el lado ERP del cruce, calculado UNA vez por request (no por
    documento). `blandos` es el índice por folio BLANDO; su porqué está más abajo, junto
    al código que lo arma. `matches` mapea (rut_canónico, N°) → {tipos de documento del
    ERP}: el TIPO viaja en el valor por el hallazgo #11 (ver FAMILIA_SII).

    Import LOCAL de MonzaContCompra (mismo patrón que los guards de monza_contabilidad):
    evita acoplar este paquete a monza_compras_contab en el arranque. La tabla
    monza_cont_compra NO tiene columna `empresa` (separación por tabla en Monza —
    verificado contra el modelo): se filtra SOLO por `anulado`.
    """
    from monza_compras_contab.models import MonzaContCompra
    filas = (db.query(MonzaContCompra.proveedor_rut, MonzaContCompra.numero_documento,
                      MonzaContCompra.tipo_doc, MonzaContCompra.origen,
                      MonzaContCompra.moneda, MonzaContCompra.monto_total_clp,
                      MonzaContCompra.monto_total)
    ...
    blandos: dict = {}
    # `montos_sin_llave` (hallazgo #14): {monto CLP redondeado → cuántas compras}.
    ...
    for rut_txt, numdoc, tipo_doc, origen, moneda, clp, total in filas:
        ...
        if canon and doc:
            ...
            #      la fecha a propósito: `fecha` es la de registro tanto como la del
            #      documento, y un rango de fechas dejaría fuera a la compra que de
            #      verdad calza. El monto es el dato que el operador no suele errar.
            #
...
def _cubeta(doc: MonzaSiiLibroDoc, matches, blandos, ruts_sin_doc, folios_sin_rut,
            montos_sin_llave):
    ...
    if tipos_erp:
        ...
        # compras identifica por (RUT, N°) y respondería 409 al botón «registrar»,
        # dejando al operador sin salida. La respuesta honesta es la tercera cubeta, con
        # la contradicción escrita.
...
def _csv_seguro(v):
    ...
    Gemelo del de wasabil_compras, duplicado a propósito: cada marca es dueña de su
    paquete y no hay imports cruzados entre ellas.
    """
...
def _reglas_por_rut(db: Session) -> dict:
    return {r.rut_canonico: r for r in db.query(MonzaSiiLibroReglaRut).all()}


def _serializar(doc: MonzaSiiLibroDoc, cubeta: str,
                regla: Optional[MonzaSiiLibroReglaRut],
...
def resumen(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    ...
    """
    ultimo_ok = (db.query(MonzaSiiLibroSyncRun)
                 .filter(MonzaSiiLibroSyncRun.exito.is_(True))
                 .order_by(MonzaSiiLibroSyncRun.id.desc()).first())
    ultimo = db.query(MonzaSiiLibroSyncRun).order_by(MonzaSiiLibroSyncRun.id.desc()).first()
    edad_horas = None
    ...
    docs = (db.query(MonzaSiiLibroDoc)
            .filter(MonzaSiiLibroDoc.estado_espejo == "ACTIVO").all())
    ...
    #     USD 45.000 aportaba «45.000 pesos» en vez de sus CLP reales) e incluía compras
    ...
    from monza_compras_contab.models import MonzaContCompra
    mes_libro = func.date_format(MonzaSiiLibroDoc.document_date, "%Y-%m")
    filas_libro = (db.query(
                       mes_libro,
                       func.sum(MonzaSiiLibroDoc.monto_efectivo),
                       # ignorados SOLO con signo + : una NC ignorada ya viaja en la
                       # columna NC — restarla dos veces inventaría diferencia.
                       func.sum(case((and_(MonzaSiiLibroDoc.decision == "ignorado",
                                           MonzaSiiLibroDoc.trx_sign == 1),
                                      MonzaSiiLibroDoc.monto_efectivo), else_=0)),
                       func.sum(case((MonzaSiiLibroDoc.trx_sign == -1,
                                      MonzaSiiLibroDoc.monto_efectivo), else_=0)))
    ...
    clp = func.coalesce(MonzaContCompra.monto_total_clp, MonzaContCompra.monto_total)
    comparable = and_(func.coalesce(MonzaContCompra.origen, "MANUAL") != "EMBARQUE",
                      func.coalesce(MonzaContCompra.moneda, "CLP") == "CLP",
                      func.coalesce(MonzaContCompra.tipo_doc, "factura")
                          .notin_(("recibo", "sin_documento")))
    mes_erp = func.date_format(MonzaContCompra.fecha, "%Y-%m")
    ...
    return {
        ...
        "desaparecidos": db.query(func.count(MonzaSiiLibroDoc.id))
                           .filter(MonzaSiiLibroDoc.estado_espejo == "DESAPARECIDO")
                           .scalar(),
...
def sincronizar_ahora(db: Session = Depends(get_db),
    ...
    """
    ultimo = (db.query(MonzaSiiLibroSyncRun)
              .order_by(MonzaSiiLibroSyncRun.id.desc()).first())
...
def bandeja(
    ...
    q = db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.estado_espejo == estado)
    if rut:
        canon = rut_canonico(rut)
        if canon is None:
            raise HTTPException(400, f"'{rut}' no tiene forma de RUT")
        q = q.filter(MonzaSiiLibroDoc.rut_emisor_canonico == canon)
    if decision == "pendiente":
        q = q.filter(MonzaSiiLibroDoc.decision.is_(None))
    elif decision in ("ignorado", "clasificado"):
        q = q.filter(MonzaSiiLibroDoc.decision == decision)
    if solo_divergentes:
        q = q.filter(MonzaSiiLibroDoc.divergente.is_(True))
    docs = q.order_by(MonzaSiiLibroDoc.document_date.desc(),
                      MonzaSiiLibroDoc.id.desc()).all()
...
def prefill_compra(doc_id: int, db: Session = Depends(get_db),
                   _: User = Depends(get_current_user)):
    """Datos del documento con los NOMBRES EXACTOS de CompraCreate (monza_compras_contab/
    ...
    bandeja mueve este documento a la cubeta ESTA solo. Espejo EXACTO del endpoint GA
    (wasabil_compras/router.py) sobre MonzaSiiLibroDoc + MonzaProveedor.
    """
    doc = db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.id == doc_id).first()
    if not doc:
    ...
    # proveedor_id si existe un MonzaProveedor con ese RUT (para preseleccionarlo en el
    # form). Se compara por CANÓNICO, no por igualdad de texto: un proveedor legado
    # puede tener el RUT guardado con puntos y aun así es el mismo emisor.
    proveedor_id = None
    if doc.rut_emisor_canonico:
        for pid, rut_txt in (db.query(MonzaProveedor.id, MonzaProveedor.rut)
                             .filter(MonzaProveedor.rut.isnot(None)).all()):
...
def decidir(doc_id: int, body: DecisionIn, db: Session = Depends(get_db),
    ...
    if body.accion == "pendiente":
        ...
        if destino == DESTINO_COSTO_VENTA:
            regla = (db.query(MonzaSiiLibroReglaRut)
                     .filter(MonzaSiiLibroReglaRut.rut_canonico == doc.rut_emisor_canonico)
                     .first())
    ...
    db.commit()
    logger.info("Libro SII Monza: doc %s (%s folio %s) → %s %s por usuario %s",
...
def _sellar_decision(doc: MonzaSiiLibroDoc, body: DecisionIn, user) -> None:
...
def listar_reglas(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    ...
    } for r in db.query(MonzaSiiLibroReglaRut).order_by(MonzaSiiLibroReglaRut.id).all()]
...
def upsert_regla(body: ReglaRutIn, db: Session = Depends(get_db),
    ...
    regla = (db.query(MonzaSiiLibroReglaRut)
             .filter(MonzaSiiLibroReglaRut.rut_canonico == canon).first())
    if not regla:
        regla = MonzaSiiLibroReglaRut(rut_canonico=canon)
        db.add(regla)
...
def asociar_rut_proveedor(proveedor_id: int, rut: str = Query(...),
                          db: Session = Depends(get_db),
                          current_user: User = Depends(get_current_user)):
    """Completa monza_proveedores.rut desde la bandeja (el camino SIN backfill de la
    Fase 0): el operador ve el documento del SII, reconoce al proveedor y lo asocia con
    un clic. Escribe canónico validado — nunca texto libre."""
    ...
    prov = db.query(MonzaProveedor).filter(MonzaProveedor.id == proveedor_id).first()
    if not prov:
    ...
    prov.rut = canon
    db.commit()
    logger.info("Libro SII Monza: proveedor %s asociado al RUT %s por usuario %s",
...
def exportar_faltantes(estado: str = Query("ACTIVO",
    ...
    if estado not in ("ACTIVO", "DESAPARECIDO"):
        raise HTTPException(400, f"estado inválido '{estado}': usa ACTIVO | DESAPARECIDO")
    docs = (db.query(MonzaSiiLibroDoc)
            .filter(MonzaSiiLibroDoc.estado_espejo == estado)
            .order_by(MonzaSiiLibroDoc.document_date.desc()).all())
    matches, blandos, ruts_sd, folios_sr, montos_sl = _llaves_erp(db)
    buf = io.StringIO()
    w = csv.writer(buf)
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition":
                 'attachment; filename="libro-sii-monza-faltantes.csv"'})
