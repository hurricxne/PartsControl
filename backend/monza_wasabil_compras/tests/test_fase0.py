"""Fase 0 del libro de compras Monza: RUT canónico + cliente de solo lectura fail-closed.

Espejo de wasabil_compras/tests/test_fase0.py (GA) sobre el paquete PROPIO de MonzaParts
(monza_wasabil_compras). LO QUE ESTA SUITE PROTEGE (las tres promesas de la Fase 0):
...
sobre la base local: monza_proveedores.rut). Molde de la casa: run() + check() + wrapper.

Corre con:  ./venv/bin/python -m pytest monza_wasabil_compras/tests/test_fase0.py -q
"""
import os
import sys
...
from monza_wasabil_compras.client import (  # noqa: E402
    WasabilComprasError, barrer_recibidos, monto_efectivo, query_recibidos_pagina,
)
from monza_wasabil_compras.rut import (  # noqa: E402
    rut_canonico, rut_formateado, rut_valido,
)
...
def run():
    ...
    try:
        ...
        for entrada, esperado in [
            ...
            ("78.121.316-0", "78121316-0"),      # el RUT de la propia empresa (Monza)
        ...
        check("1v módulo 11: 78.121.316-0 es válido", rut_valido("78.121.316-0"))
        check("1w módulo 11: 78.121.316-1 NO es válido (DV cambiado)",
              not rut_valido("78.121.316-1"))
    ...
    from migrations.monza_proveedor_rut import run as migrar
    migrar()
    migrar()  # segunda corrida: no debe reventar ni duplicar
    from sqlalchemy import inspect
    from database import engine
    cols = {c["name"] for c in inspect(engine).get_columns("monza_proveedores")}
    check("7 monza_proveedores.rut existe tras migrar dos veces", "rut" in cols, cols)
...
def test_monza_wasabil_compras_fase0():
    run()
