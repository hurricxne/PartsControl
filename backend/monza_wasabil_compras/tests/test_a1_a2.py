"""Fases A1+A2 del libro de compras Monza: espejo idempotente + bandeja de tres cubetas.

Espejo de wasabil_compras/tests/test_a1_a2.py (GA) sobre el paquete PROPIO de MonzaParts.
...
ADAPTACIONES Monza vs la suite GA:
  · Empresa del override: 'automotriz'; el candado (§8) es el INVERSO — un usuario de
    'mineria' recibe 403.
  · El cruce ERP usa MonzaContCompra (monza_compras_contab.models) — SIN columna
    `empresa` (separación por tabla en Monza): las compras de mentira se crean sin ella.
  · El proveedor de §9 es MonzaProveedor (monza_models), que exige `code` UNIQUE.
...
REVISIÓN ADVERSARIAL 2026-08-06 (hallazgos confirmados, ver el arreglo en cada módulo):
  ...
  · §15 (#13) MONZA_CONTAB_ENABLED=false + create_all sobre BD vacía NO revienta
       (grafo de FKs cerrado en models.py) y (#17) raw_json deferred.
...
nueva. El cruce usa compras REALES de mentira en monza_cont_compra (MARK propio), y §18
usa el alta REAL de Compras y Pagos (el router se monta acá) porque el anti-duplicado del
alta y la advertencia de la bandeja son las dos mitades del mismo control.

Corre con:  ./venv/bin/python -m pytest monza_wasabil_compras/tests/test_a1_a2.py -q
"""
import csv
import io
import os
...
from monza_models import MonzaProveedor  # noqa: E402
import monza_wasabil_compras.client as cli  # noqa: E402
import monza_wasabil_compras.sync as sync_mod  # noqa: E402
from monza_wasabil_compras.models import (  # noqa: E402
    MonzaSiiLibroDoc, MonzaSiiLibroMatch, MonzaSiiLibroReglaRut, MonzaSiiLibroSyncRun,
)
from monza_wasabil_compras.router import router as sii_router  # noqa: E402
from monza_wasabil_compras.sync import ejecutar_barrido  # noqa: E402

BACKEND_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
# monza_compras_contab.models ya importa monza_embarques_pricing.models por su cuenta
# (FK monza_cont_compra.emb_pricing_gasto_id) — no hace falta importarlo aparte.
...
MARK = "test-mzsiilibro"
CURRENT = {"id": 1, "empresa": "automotriz"}

app = FastAPI()
app.include_router(sii_router)
app.include_router(compras_router)   # sondas §18 (anti-dup del alta; trae su prefijo)
app.dependency_overrides[get_current_user] = lambda: SimpleNamespace(
    id=CURRENT["id"], empresa=CURRENT["empresa"], email=f"{MARK}@test.invalid")
client = TestClient(app)

API = "/api/monza/sii-libro"
API_COMPRAS = "/api/monza/compras-contab"   # el alta que prueba §18
_fails = []
...
def _limpiar():
    ...
    try:
        S = "fetch"
        # Los CRUCES del matcher primero: la FK monza_sii_libro_match.doc_id es ON DELETE
        ...
        ids_mark = [r[0] for r in db.query(MonzaSiiLibroDoc.id)
                    .filter(MonzaSiiLibroDoc.uuid.like(f"{MARK}%")).all()]
        if ids_mark:
            db.query(MonzaSiiLibroMatch).filter(
                MonzaSiiLibroMatch.doc_id.in_(ids_mark)).delete(synchronize_session=S)
        db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.uuid.like(f"{MARK}%")).delete(
            synchronize_session=S)
        # Los runs no llevan MARK: se borran TODOS los runs que no estén referenciados
        # por ningún doc vivo y sean del usuario de prueba (mismo criterio que la suite
        # GA — la bitácora real siempre tiene docs apuntándole o errores que se conservan).
        referenciados = {r[0] for r in db.query(MonzaSiiLibroDoc.ultimo_run_id)
                         .filter(MonzaSiiLibroDoc.ultimo_run_id.isnot(None)).all()}
        q = db.query(MonzaSiiLibroSyncRun)
        if referenciados:
            q = q.filter(~MonzaSiiLibroSyncRun.id.in_(referenciados))
        q.filter(MonzaSiiLibroSyncRun.usuario_id == CURRENT["id"]).delete(
            synchronize_session=S)
        db.query(MonzaSiiLibroReglaRut).filter(
            MonzaSiiLibroReglaRut.motivo.like(f"{MARK}%")).delete(synchronize_session=S)
        db.query(MonzaContCompra).filter(MonzaContCompra.acreedor.like(f"{MARK}%")).delete(
            synchronize_session=S)
        db.query(MonzaProveedor).filter(MonzaProveedor.nombre.like(f"{MARK}%")).delete(
            synchronize_session=S)
        db.commit()
    finally:
        db.close()
...
def _verificar_limpieza():
    ...
    try:
        restos = (
            db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.uuid.like(f"{MARK}%")).count()
            + db.query(MonzaContCompra).filter(MonzaContCompra.acreedor.like(f"{MARK}%")).count()
            + db.query(MonzaProveedor).filter(MonzaProveedor.nombre.like(f"{MARK}%")).count())
...
def _fila(uuid):
    db = SessionLocal()
    try:
        return db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.uuid == uuid).first()
    finally:
        db.close()
...
def run():
    ...
    # abortaría. La sonda §14c lo re-activa con su valor real y lo prueba a propósito.
    ...
    try:
        ...
        db = SessionLocal()
        n = db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.uuid.like(f"{MARK}%")).count()
        db.close()
        ...
        # SIN columna empresa: monza_cont_compra separa por tabla (adaptación Monza).
        db.add(MonzaContCompra(acreedor=f"{MARK} RAMAQ",
        ...
        db = SessionLocal()
        db.add(MonzaContCompra(acreedor=f"{MARK} sin rut",
        ...
        # ── 8) Candado de empresa (INVERSO al de GA) ─────────────────────────────────
        CURRENT["empresa"] = "mineria"
        r = client.get(f"{API}/resumen")
        check("8 un usuario de MachParts/Grupo AM recibe 403", r.status_code == 403,
              r.status_code)
        CURRENT["empresa"] = "automotriz"

        # ── 9) Asociar RUT al proveedor (el camino sin backfill) ─────────────────────
        db = SessionLocal()
        prov = MonzaProveedor(code=f"{MARK}-provx", nombre=f"{MARK} PROVEEDOR X")
        db.add(prov)
        db.commit()
        pid = prov.id
        db.close()
        r = client.post(f"{API}/proveedores/{pid}/asociar-rut",
                        params={"rut": "78.121.316-1"})
        check("9a un RUT con módulo 11 malo se rechaza", r.status_code == 400, r.text[:160])
        r = client.post(f"{API}/proveedores/{pid}/asociar-rut",
                        params={"rut": "78.121.316-0"})
        check("9b uno válido se asocia canónico", r.status_code == 200
              and r.json()["rut"] == "78121316-0", r.text[:160])
        r = client.post(f"{API}/proveedores/{pid}/asociar-rut",
        ...
        # Espejo de §10 de la suite GA. RUT SINTÉTICO a propósito: ningún proveedor real
        # del entorno lo tiene, así el proveedor_id nulo/preseleccionado es determinista.
        # El d5 lleva porción EXENTA: la sonda de que el exento viaja dentro del neto
        # (neto + IVA == total del doc).
        ...
        # ADAPTACIÓN Monza: MonzaProveedor exige `code` UNIQUE.
        db = SessionLocal()
        prov2 = MonzaProveedor(code=f"{MARK}-prefill", nombre=f"{MARK} PREFILL SA",
                               rut="78.901.230-5")
        db.add(prov2)
        db.commit()
        pid2 = prov2.id
        ...
        db = SessionLocal()
        db.add(MonzaContCompra(acreedor=f"{MARK} BLANDO SPA",
        ...
        # d9 + su compra son la SONDA #9 del lado bandeja (ver 12f-bis): el RUT del ERP
        ...
        db = SessionLocal()
        db.add(MonzaContCompra(acreedor=f"{MARK} TIPOS SPA",
        ...
        db.add(MonzaContCompra(acreedor=f"{MARK} RUT SUCIO",
        ...
        db.add(MonzaContCompra(acreedor=f"{MARK} CAJA CHICA",
        ...
        db.add(MonzaContCompra(acreedor=f"{MARK} SIN LLAVE",
                               proveedor_rut=None, numero_documento=None,
                               tipo_doc="factura", moneda="CLP", origen="MANUAL",
                               monto_total=238000, monto_total_clp=238000,
                               fecha=date(2026, 7, 15)))
        ...
        db = SessionLocal()
        f1 = db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.uuid == U(1)).first()
        ...
        # #13: el escenario LITERAL del hallazgo — MONZA_CONTAB_ENABLED=false, se
        # importan SOLO los módulos que main.py monta SIEMPRE (fuera del gate) y se
        # corre create_all sobre una BD VACÍA (la instalación fresca que el gate
        # protege). Sin el cierre del grafo de FKs en models.py: NoReferencedTableError
        # ('monza_cont_adelanto') y el backend ENTERO no arranca.
        codigo = ("import monza_wasabil_compras.router\n"
                  "import monza_wasabil_compras.router_match\n"
                  "from models.models import Base\n"
                  "from sqlalchemy import create_engine\n"
                  "Base.metadata.create_all(create_engine('sqlite://'))\n")
        env = dict(os.environ, MONZA_CONTAB_ENABLED="false")
        res = subprocess.run([sys.executable, "-c", codigo], capture_output=True,
                             text=True, cwd=BACKEND_DIR, env=env, timeout=120)
        check("15c SONDA #13: con el gate contable APAGADO, los imports incondicionales "
              "+ create_all sobre BD vacía NO revientan (el apagado de emergencia ya "
              "no tumba las dos marcas)", res.returncode == 0,
              (res.stderr or res.stdout)[-300:])
        codigo2 = ("import monza_wasabil_compras.models\n"
                   "from models.models import Base\n"
                   "from sqlalchemy import create_engine\n"
                   "Base.metadata.create_all(create_engine('sqlite://'))\n")
        res2 = subprocess.run([sys.executable, "-c", codigo2], capture_output=True,
                              text=True, cwd=BACKEND_DIR, timeout=120)
        check("15d SONDA import AISLADO de los models (bajo confirmado: el import "
              "defensivo debe cerrar el grafo COMPLETO)", res2.returncode == 0,
              (res2.stderr or res2.stdout)[-300:])

        # ── 16) CSV: inyección de fórmulas neutralizada (lente de seguridad) ─────────
        # Espejo del §16 de Grupo AM. El nombre del emisor lo escribe un TERCERO (viene
        # del SII); Excel ejecuta como fórmula la celda que empieza con = + - @, así que
        # un proveedor con nombre hostil corre código en el computador del contador que
        # abre el reporte. Va AL FINAL a propósito: pisa los nombres de los docs del MARK
        # y no necesita restaurarlos (la limpieza los borra).
        ...
        try:
            db.query(MonzaSiiLibroDoc).filter(
                MonzaSiiLibroDoc.uuid.like(f"{MARK}%")).update(
                {"receiver_name": HOSTIL}, synchronize_session=False)
            db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.uuid == U(1)).update(
                {"receiver_name": HOSTIL_ESP}, synchronize_session=False)
            db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.uuid == U(31)).update(
        ...
        db = SessionLocal()
        try:
            run_ok = MonzaSiiLibroSyncRun(origen="manual", exito=True,
        ...
        db = SessionLocal()
        try:
            run = (db.query(MonzaSiiLibroSyncRun)
                   .filter(MonzaSiiLibroSyncRun.id == run_id).first())
            run.exito = False
            db.commit()
        finally:
            db.close()
        ...
        r = client.post(API_COMPRAS, json=dict(
        ...
        db = SessionLocal()
        n_sucias = (db.query(MonzaContCompra)
                    .filter(MonzaContCompra.acreedor == f"{MARK} RUT SUCIO",
                            MonzaContCompra.anulado.is_(False)).count())
        db.close()
...
def test_monza_sii_libro_a1_a2():
    run()
