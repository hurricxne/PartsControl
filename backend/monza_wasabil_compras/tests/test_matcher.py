"""Matcher banco ↔ libro SII ↔ egresos MONZA: las 22 reglas de la spec, cada una con
su sonda — espejo de wasabil_compras/tests/test_matcher.py (GA) sobre las tablas y
routers Monza (monza_tesoreria / monza_compras_contab / monza_wasabil_compras).
...
sobrevive al re-import de cartola (§R5), idempotencia N corridas (§R5). Más el candado
INVERSO de marca: un usuario de MachParts (mineria) recibe 403.
...
Corre con:  ./venv/bin/python -m pytest monza_wasabil_compras/tests/test_matcher.py -q -s
"""
import json
import os
import re
...
from monza_contabilidad.models import (  # noqa: E402
    MonzaContCobranza, MonzaContFacturaCliente,
)
from monza_compras_contab.models import (  # noqa: E402
    MonzaContCompra, MonzaContEgreso, MonzaContEgresoDetalle,
)
from monza_compras_contab.router import router as compras_router  # noqa: E402 (H#1)
from monza_tesoreria.models import (  # noqa: E402
    MonzaTesCartola, MonzaTesConciliacion, MonzaTesCuentaBancaria,
    MonzaTesMovimiento,
)
from monza_tesoreria.router import router as tes_router  # noqa: E402
from monza_wasabil_compras.glosa import (  # noqa: E402
    extraer_numero_operacion, extraer_ruts_de_glosa, fuerza_nombre, tokens_coinciden,
)
from monza_wasabil_compras.matcher import comparar_lado  # noqa: E402
from monza_wasabil_compras.models import (  # noqa: E402
    MonzaSiiLibroDoc, MonzaSiiLibroMatch, MonzaSiiLibroReglaRut,
    MonzaSiiLibroSyncRun, MonzaSiiMatchConfig, MonzaSiiMatchEtiquetaMov,
    MonzaSiiMatchRun,
)
from monza_wasabil_compras.router_match import router as match_router  # noqa: E402
from monza_wasabil_compras.rut import rut_canonico  # noqa: E402

Base.metadata.create_all(bind=engine, checkfirst=True)

MARK = "test-monza-siimatch"
# monza_sii_libro_sync_run.origen es String(20): el MARK completo + '-fake' no cabe.
FAKE_ORIGEN = "tst-monza-fake"
CURRENT = {"id": 1, "empresa": "automotriz"}
HOY = date.today()

app = FastAPI()
app.include_router(match_router)
app.include_router(tes_router)          # el router Monza ya trae su prefijo completo
app.include_router(compras_router)      # sonda anti-dup blando (H#1)
app.dependency_overrides[get_current_user] = lambda: SimpleNamespace(
    id=CURRENT["id"], empresa=CURRENT["empresa"], email=f"{MARK}@test.invalid")
client = TestClient(app)

API = "/api/monza/sii-libro/match"
...
def mk_mov(monto, *, glosa="", tipo="cargo", dias=0, ref=None, cuenta=None,
    ...
    db = _s()
    try:
        m = MonzaTesMovimiento(
            cuenta_id=cuenta or IDS["cuenta"],
...
def mk_compra(rut, numdoc, monto, estado="pendiente"):
    db = _s()
    try:
        c = MonzaContCompra(acreedor=f"{MARK} prov", proveedor_rut=rut,
...
def mk_egreso(monto, *, medio="transferencia", numop=None, benef_rut=None,
    ...
    db = _s()
    try:
        e = MonzaContEgreso(fecha=fecha or HOY, medio=medio,
                            numero_operacion=numop, beneficiario_rut=benef_rut,
                            glosa=MARK, monto_total_clp=monto)
        db.add(e)
        db.flush()
        for cid, m in detalles:
            db.add(MonzaContEgresoDetalle(egreso_id=e.id, compra_id=cid, monto_clp=m))
        db.commit()
        return e.id
    finally:
        db.close()
...
def conciliar_interno(mov_id, egreso_id, monto):
    """El enlace banco↔egreso como lo deja Tesorería Monza (la tercera pata)."""
    db = _s()
    try:
        db.add(MonzaTesConciliacion(movimiento_id=mov_id, egreso_id=egreso_id,
                                    monto_conciliado_clp=monto))
        db.query(MonzaTesMovimiento).filter(MonzaTesMovimiento.id == mov_id).update(
            {"conciliado": True, "conciliado_at": datetime.utcnow()})
        db.query(MonzaContEgreso).filter(MonzaContEgreso.id == egreso_id).update(
            {"conciliado": True})
        db.commit()
    finally:
        db.close()
...
def set_config(clave, valor):
    db = _s()
    try:
        fila = db.query(MonzaSiiMatchConfig).filter(
            MonzaSiiMatchConfig.clave == clave).first()
        if fila is None:
            fila = MonzaSiiMatchConfig(clave=clave, valor=json.dumps(valor))
            db.add(fila)
        else:
...
def matches_de(mov_id=None, doc_id=None):
    ...
    db = _s()
    try:
        q = db.query(MonzaSiiLibroMatch)
        if mov_id is not None:
            q = q.filter(MonzaSiiLibroMatch.movimiento_id == mov_id)
        if doc_id is not None:
            q = q.filter(MonzaSiiLibroMatch.doc_id == doc_id)
        return [SimpleNamespace(
            ...
            for m in q.order_by(MonzaSiiLibroMatch.id).all()]
    finally:
        db.close()
...
def mov_snap(mov_id):
    db = _s()
    try:
        m = db.query(MonzaTesMovimiento).filter(
            MonzaTesMovimiento.id == mov_id).first()
...
def pisar_doc(doc_id, **campos):
    ...
    try:
        ...
        db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.id == doc_id).update(campos)
        db.commit()
    finally:
        db.close()
...
def _limpiar():
    ...
    try:
        S = "fetch"
        doc_ids = [r[0] for r in db.query(MonzaSiiLibroDoc.id)
                   .filter(MonzaSiiLibroDoc.uuid.like(f"{MARK}%")).all()]
        cuenta_ids = [r[0] for r in db.query(MonzaTesCuentaBancaria.id)
                      .filter(MonzaTesCuentaBancaria.banco.like(f"{MARK}%")).all()]
        mov_ids = [r[0] for r in db.query(MonzaTesMovimiento.id)
        ...
        if doc_ids or mov_ids:
            ...
            cond = []
            if doc_ids:
                cond.append(MonzaSiiLibroMatch.doc_id.in_(doc_ids))
            if mov_ids:
                cond.append(MonzaSiiLibroMatch.movimiento_id.in_(mov_ids))
            from sqlalchemy import or_
            q.filter(or_(*cond)).delete(synchronize_session=S)
        if mov_ids:
            db.query(MonzaSiiMatchEtiquetaMov).filter(
                MonzaSiiMatchEtiquetaMov.movimiento_id.in_(mov_ids)).delete(
                synchronize_session=S)
            db.query(MonzaTesConciliacion).filter(
                MonzaTesConciliacion.movimiento_id.in_(mov_ids)).delete(
                synchronize_session=S)
            db.query(MonzaTesMovimiento).filter(
                MonzaTesMovimiento.id.in_(mov_ids)).delete(synchronize_session=S)
        if cuenta_ids:
            db.query(MonzaTesCartola).filter(
                MonzaTesCartola.cuenta_id.in_(cuenta_ids)).delete(
                synchronize_session=S)
            db.query(MonzaTesCuentaBancaria).filter(
                MonzaTesCuentaBancaria.id.in_(cuenta_ids)).delete(
                synchronize_session=S)
        if doc_ids:
            db.query(MonzaSiiLibroDoc).filter(
                MonzaSiiLibroDoc.id.in_(doc_ids)).delete(synchronize_session=S)
        db.query(MonzaContEgreso).filter(
            MonzaContEgreso.glosa.like(f"{MARK}%")).delete(synchronize_session=S)
        db.query(MonzaContCompra).filter(
            MonzaContCompra.acreedor.like(f"{MARK}%")).delete(synchronize_session=S)
        # Facturas: cobranzas primero (sin depender del cascade del ORM Monza).
        fac_ids = [r[0] for r in db.query(MonzaContFacturaCliente.id)
                   .filter(MonzaContFacturaCliente.numero_factura
                           .like(f"{MARK}%")).all()]
        if fac_ids:
            db.query(MonzaContCobranza).filter(
                MonzaContCobranza.factura_id.in_(fac_ids)).delete(
                synchronize_session=S)
            db.query(MonzaContFacturaCliente).filter(
                MonzaContFacturaCliente.id.in_(fac_ids)).delete(
                synchronize_session=S)
        db.query(MonzaSiiLibroReglaRut).filter(
            MonzaSiiLibroReglaRut.motivo.like(f"{MARK}%")).delete(
            synchronize_session=S)
        db.query(MonzaSiiMatchRun).filter(
            MonzaSiiMatchRun.usuario_id == CURRENT["id"]).delete(
            synchronize_session=S)
        db.query(MonzaSiiMatchConfig).filter(
            MonzaSiiMatchConfig.clave == "rut_en_glosa_activo").delete(
            synchronize_session=S)
        db.query(MonzaSiiLibroSyncRun).filter(
            MonzaSiiLibroSyncRun.origen == FAKE_ORIGEN).delete(
            synchronize_session=S)
        db.commit()
    finally:
        db.close()
...
def _verificar_limpieza():
    db = _s()
    try:
        restos = (db.query(MonzaSiiLibroDoc)
                  .filter(MonzaSiiLibroDoc.uuid.like(f"{MARK}%")).count()
                  + db.query(MonzaTesCuentaBancaria).filter(
                      MonzaTesCuentaBancaria.banco.like(f"{MARK}%")).count()
                  + db.query(MonzaContEgreso).filter(
                      MonzaContEgreso.glosa.like(f"{MARK}%")).count()
                  + db.query(MonzaContCompra).filter(
                      MonzaContCompra.acreedor.like(f"{MARK}%")).count())
...
def run():
    ...
    try:
        ...
        db = _s()
        cta = MonzaTesCuentaBancaria(banco=f"{MARK} Santander", moneda="CLP")
        cta_usd = MonzaTesCuentaBancaria(banco=f"{MARK} USD", moneda="USD")
        db.add_all([cta, cta_usd])
        db.commit()
        IDS["cuenta"], IDS["cuenta_usd"] = cta.id, cta_usd.id
        car = MonzaTesCartola(cuenta_id=cta.id, nombre=f"{MARK} c1")
        db.add(car)
        db.commit()
        IDS["cartola"] = car.id
        db.close()
        ...
        db = _s()
        c5 = MonzaTesCartola(cuenta_id=IDS["cuenta"], nombre=f"{MARK} c5")
        db.add(c5)
        db.commit()
        c5_id = c5.id
        db.close()
        ...
        db = _s()
        c5b = MonzaTesCartola(cuenta_id=IDS["cuenta"], nombre=f"{MARK} c5b")
        db.add(c5b)
        db.commit()
        c5b_id = c5b.id
        db.close()
        ...
        db = _s()
        antes = db.query(MonzaSiiLibroMatch).count()
        conf_antes = db.query(MonzaSiiLibroMatch).filter(
            MonzaSiiLibroMatch.estado == "confirmado").count()
        desc_antes = db.query(MonzaSiiLibroMatch).filter(
            MonzaSiiLibroMatch.estado == "descartado").count()
        db.close()
        correr()
        correr()
        db = _s()
        despues = db.query(MonzaSiiLibroMatch).count()
        conf_despues = db.query(MonzaSiiLibroMatch).filter(
            MonzaSiiLibroMatch.estado == "confirmado").count()
        desc_despues = db.query(MonzaSiiLibroMatch).filter(
            MonzaSiiLibroMatch.estado == "descartado").count()
        db.close()
        ...
        db = _s()
        fac = MonzaContFacturaCliente(numero_factura=f"{MARK}-F1",
                                      monto_bruto=238_007)
        db.add(fac)
        db.flush()
        cob = MonzaContCobranza(factura_id=fac.id, monto=238_007,
                                medio="transferencia", fecha=HOY)
        db.add(cob)
        db.commit()
        cob_id = cob.id
        db.close()
        ...
        dnc = mk_doc(141, rut="76310394-3", nombre="SERVICIOS AUTOMOTRICES MAIPU LTDA",
                     monto=45_220_013, signo=-1)
        ab2 = mk_mov(45_220_013, tipo="abono",
                     glosa="TRANSFERENCIA DE SERVICIOS AUTOMOTRICES MAIPU")
        correr()
        ...
        # Intercompañía Monza: GRUPO AM (77977813-4) — el espejo del 78121316-0 de GA.
        cri = mk_compra("77977813-4", "8877M", 5_500_057)
        eri = mk_egreso(5_500_057, detalles=[(cri, 5_500_057)])
        mri = mk_mov(5_500_057, glosa="PAGO 77.977.813-4")
        conciliar_interno(mri, eri, 5_500_057)
        dri = mk_doc(150, rut="77977813-4", nombre="GRUPO AM SPA INTERCOMPANIA",
        ...
        db = _s()
        db.add(MonzaSiiLibroReglaRut(rut_canonico="76310395-1", nivel="BLOQUEADO",
        ...
        db = _s()
        et = db.query(MonzaSiiMatchEtiquetaMov).filter(
            MonzaSiiMatchEtiquetaMov.movimiento_id == mn).first()
        ...
        correr()
        db = _s()
        etl = db.query(MonzaSiiMatchEtiquetaMov).filter(
            MonzaSiiMatchEtiquetaMov.movimiento_id == ml).first()
        db.close()
        ...
        correr()
        db = _s()
        et18 = db.query(MonzaSiiMatchEtiquetaMov).filter(
            MonzaSiiMatchEtiquetaMov.movimiento_id == m18).first()
        db.close()
        ...
        db = _s()
        fake = MonzaSiiLibroSyncRun(origen=FAKE_ORIGEN,
        ...
        db.commit()
        db.close()
        # sonda estática: JAMÁS FOR UPDATE sobre monza_sii_libro_doc
        ...
        malas = [i for i, ln in enumerate(lineas) if "with_for_update" in ln
                 and any("MonzaSiiLibroDoc" in lineas[j]
                         for j in range(max(0, i - 4), i + 1))]
        check("22b sonda estática: cero with_for_update sobre MonzaSiiLibroDoc en el "
        ...
        check("22c y el doc del auto se relee con populate_existing SIN lock",
              re.search(r"MonzaSiiLibroDoc\.id == p\[\"doc\"\]\.id\)\s*\n\s*"
                        r"\.populate_existing\(\)\.first\(\)", src) is not None)

        # ═══ §H — revisión adversarial 2026-08-06: arreglos con sonda propia ══════
        # Espejo Monza de las sondas §H de GA (duplicación deliberada entre marcas).
        ...
        db = _s()
        db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.id == dh7).update(
            {"destino": "cc:honorarios"})
        db.commit()
        db.close()
        from monza_wasabil_compras.matcher import CONFIG_DEFAULTS
        ...
        check("H8c sonda estática: el confirm y _insertar_auto serializan el lado "
              ...
              re.search(r"MonzaSiiLibroMatch\.doc_id == did\)\s*\n\s*"
                        r"\.with_for_update\(\)", src_rm) is not None
              and re.search(r"MonzaSiiLibroMatch\.doc_id == p\[\"doc\"\]\.id\)"
                            r"\s*\n\s*\.with_for_update\(\)", src_mm) is not None)

        # ── H#9: guards de Tesorería sobre matches confirmados del libro
        db = _s()
        c9x = MonzaTesCartola(cuenta_id=IDS["cuenta"], nombre=f"{MARK} c9")
        db.add(c9x)
        db.commit()
        c9_id = c9x.id
        db.close()
        ...
        db = _s()
        db.query(MonzaTesMovimiento).filter(MonzaTesMovimiento.id == mh9).update(
        ...
        # `from monza_wasabil_compras.models import …` levanta ImportError, mientras
        # find_spec('monza_wasabil_compras') sigue viendo el paquete instalado → 503.
        _mod_saboteado = sys.modules.get("monza_wasabil_compras.models")
        sys.modules["monza_wasabil_compras.models"] = None
        try:
            r = client.delete(f"{TES}/movimientos/{mh9}")
        finally:
            if _mod_saboteado is not None:
                sys.modules["monza_wasabil_compras.models"] = _mod_saboteado
            else:
                sys.modules.pop("monza_wasabil_compras.models", None)
        ...
        # pantalla).
        db = _s()
        c9f = MonzaTesCartola(cuenta_id=IDS["cuenta"], nombre=f"{MARK} c9f")
        db.add(c9f)
        db.commit()
        c9f_id = c9f.id
        db.close()
        ...
        db = _s()
        fila_m10 = (db.query(MonzaTesMovimiento.id)
                    .filter(MonzaTesMovimiento.cartola_id == cart_h10).first())
        db.close()
        ...
        db = _s()
        hay_run_pc = (db.query(MonzaSiiMatchRun)
                      .filter(MonzaSiiMatchRun.origen == "post_cartola").first()
                      is not None)
        db.close()
        ...
        # con Session SÍNCRONA: en línea bloqueaba el EVENT LOOP y congelaba TODAS las
        # peticiones del proceso mientras el operador miraba girar el spinner.
        ...
        src_tes = open(os.path.join(os.path.dirname(__file__), "..", "..",
                                    "monza_tesoreria", "router.py")).read()
        ...
        check("H10c2 SONDA #4: importar cartola AGENDA la corrida "
              ...
              cuerpo_importar[-400:])
        from monza_tesoreria.router import _correr_matcher_post_cartola
        db = _s()
        n_pc_antes = (db.query(MonzaSiiMatchRun)
                      .filter(MonzaSiiMatchRun.origen == "post_cartola").count())
        db.close()
        _correr_matcher_post_cartola(None)
        db = _s()
        n_pc_post = (db.query(MonzaSiiMatchRun)
                     .filter(MonzaSiiMatchRun.origen == "post_cartola").count())
        db.close()
        ...
        db = _s()
        ids_pb_antes = {x[0] for x in db.query(MonzaSiiMatchRun.id)
                        .filter(MonzaSiiMatchRun.origen == "post_barrido").all()}
        db.add(MonzaSiiLibroSyncRun(origen=FAKE_ORIGEN,
                                    iniciado_at=datetime.utcnow(),
                                    terminado_at=datetime.utcnow(), exito=True))
        db.commit()
        db.close()
        sched._matcher_post_barrido_monza(datetime.utcnow() - timedelta(minutes=5))
        db = _s()
        ids_pb_1 = {x[0] for x in db.query(MonzaSiiMatchRun.id)
                    .filter(MonzaSiiMatchRun.origen == "post_barrido").all()}
        db.close()
        ...
        sched._matcher_post_barrido_monza(datetime.utcnow() + timedelta(hours=1))
        db = _s()
        ids_pb_2 = {x[0] for x in db.query(MonzaSiiMatchRun.id)
                    .filter(MonzaSiiMatchRun.origen == "post_barrido").all()}
        if ids_pb_2 - ids_pb_antes:
            db.query(MonzaSiiMatchRun).filter(
                MonzaSiiMatchRun.id.in_(list(ids_pb_2 - ids_pb_antes))).delete(
        ...
        r1 = client.post("/api/monza/compras-contab", json=pay1)
        ...
        r2 = client.post("/api/monza/compras-contab", json=pay2)
        ...
        r3 = client.post("/api/monza/compras-contab", json=pay3)
        ...
        import monza_wasabil_compras.matcher as _mm
        db = SessionLocal()
        try:
            n_antes = db.query(MonzaSiiMatchRun).count()
            ...
            nacida = (db.query(MonzaSiiMatchRun).filter(MonzaSiiMatchRun.id > 0)
                      .order_by(MonzaSiiMatchRun.id.desc()).first())
            ...
            ultima_bd = (db.query(MonzaSiiMatchRun)
                         .order_by(MonzaSiiMatchRun.id.desc()).first())
            r = client.get(f"{API}/resumen")
            ...
            if nacida is not None and db.query(MonzaSiiMatchRun).count() > n_antes:
                db.query(MonzaSiiMatchRun).filter(MonzaSiiMatchRun.id == nacida.id).delete(
                    synchronize_session=False)
                db.commit()
            check("24g limpieza de la sonda (la bitácora vuelve a su tamaño)",
                  db.query(MonzaSiiMatchRun).count() == n_antes,
                  (db.query(MonzaSiiMatchRun).count(), n_antes))
        finally:
            db.close()

        # ═══ candado de empresa (INVERSO al de GA) ════════════════════════════════
        CURRENT["empresa"] = "mineria"
        r = client.get(f"{API}/resumen")
        check("23 un usuario de MachParts (mineria) recibe 403", r.status_code == 403,
              r.status_code)
        CURRENT["empresa"] = "automotriz"
...
def test_monza_matcher_banco_libro():
    run()
