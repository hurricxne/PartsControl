"""El barrido del espejo Monza — COMPLETO todas las noches, idempotente, sin DELETE jamás.

Espejo deliberado de wasabil_compras/sync.py (GA) con imports PROPIOS del paquete Monza.
...
from .models import (
    CAMPOS_HASH, MonzaSiiLibroDoc, MonzaSiiLibroSyncRun, hash_contenido,
)
...
def _run_en_curso(db: Session) -> Optional[MonzaSiiLibroSyncRun]:
    limite = datetime.utcnow() - timedelta(minutes=RUN_TIMEOUT_MIN)
    return (db.query(MonzaSiiLibroSyncRun)
            .filter(MonzaSiiLibroSyncRun.terminado_at.is_(None),
                    MonzaSiiLibroSyncRun.iniciado_at > limite)
            .first())


def _superado_por_otro_run(db: Session,
                           run: MonzaSiiLibroSyncRun) -> Optional[MonzaSiiLibroSyncRun]:
    ...
    Cierre elegido: FENCING al escribir, no heartbeat — monza_sii_libro_sync_run NO
    tiene columna updated_at y agregarla exigiría un ALTER que create_all no hace (la
    trampa 1054 que narra el propio checklist de deploy). El fencing cubre el daño real
    del hallazgo: si un run con id MAYOR ya existe (vivo o terminado), los datos de red
    de ESTE run son más viejos que los del otro → este run se retira SIN escribir. Se
    consulta dos veces: antes de abrir la escritura y justo antes del sweep."""
    return (db.query(MonzaSiiLibroSyncRun)
            .filter(MonzaSiiLibroSyncRun.id > run.id)
            .order_by(MonzaSiiLibroSyncRun.id)
            .first())


def _abortar_run(db: Session, run: MonzaSiiLibroSyncRun, origen: str, motivo: str) -> None:
    ...
    db.commit()
    logger.error("Barrido libro SII Monza (%s) ABORTADO: %s", origen, motivo)


def ejecutar_barrido(db: Session, *, origen: str,
                     usuario_id: Optional[int] = None) -> MonzaSiiLibroSyncRun:
    ...
    run = MonzaSiiLibroSyncRun(origen=origen, usuario_id=usuario_id,
    ...
    try:
        ...
        db.commit()
        logger.error("Barrido libro SII Monza (%s) ABORTADO: %s", origen, e)
        return run
    ...
    try:
        ...
        activos_ventana = (db.query(func.count(MonzaSiiLibroDoc.id))
                           .filter(MonzaSiiLibroDoc.estado_espejo == "ACTIVO",
                                   MonzaSiiLibroDoc.document_date >= desde)
                           .scalar() or 0)
        por_desaparecer = (db.query(func.count(MonzaSiiLibroDoc.id))
                           .filter(MonzaSiiLibroDoc.estado_espejo == "ACTIVO",
                                   MonzaSiiLibroDoc.document_date >= desde,
                                   (MonzaSiiLibroDoc.ultimo_run_id.is_(None))
                                   | (MonzaSiiLibroDoc.ultimo_run_id < run.id))
                           .scalar() or 0)
        ...
        desaparecidos = (db.query(MonzaSiiLibroDoc)
                         .filter(MonzaSiiLibroDoc.estado_espejo == "ACTIVO",
                                 MonzaSiiLibroDoc.document_date >= desde,
                                 (MonzaSiiLibroDoc.ultimo_run_id.is_(None))
                                 | (MonzaSiiLibroDoc.ultimo_run_id < run.id))
        ...
        db.commit()
        logger.info("Barrido libro SII Monza (%s) OK: %s docs (%s nuevos, %s act., "
                    "%s desap., %s malformados)", origen, len(documentos), nuevos,
                    actualizados, desaparecidos, len(malformados))
        ...
        db.commit()
        logger.exception("Barrido libro SII Monza (%s): fallo escribiendo el espejo",
                         origen)
    return run
...
def _upsert_doc(db: Session, doc: dict, run_id: int) -> Tuple[bool, Optional[str]]:
    ...
    fila = db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.uuid == doc["uuid"]).first()
    hash_nuevo = hash_contenido(doc)
    es_nuevo = fila is None
    if es_nuevo:
        fila = MonzaSiiLibroDoc(uuid=doc["uuid"])
        db.add(fila)
    ...
    try:
        ...
        logger.warning("Barrido libro SII Monza: documento malformado espejado con "
                       "monto NULL — %s", aviso)
...
def barrido_nocturno() -> None:
    ...
    del libro Monza jamás deja al sistema sin el resto de las alertas del día (invariantes
    del job, ver scheduler.py). Usa el esta_configurado PROPIO (token Monza): sin
    WASABIL_API_TOKEN_MONZA el barrido se omite sin error."""
    from database import SessionLocal
    from .client import esta_configurado
    if not esta_configurado():
        logger.info("Barrido libro SII Monza: Wasabil sin token — se omite (no es un error)")
        return
    db = SessionLocal()
    try:
        ejecutar_barrido(db, origen="nocturno")
    except BarridoEnCurso as e:
        logger.warning("Barrido libro SII Monza nocturno: %s", e)
    finally:
        db.close()
