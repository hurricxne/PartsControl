"""API del matcher banco ↔ libro SII ↔ egresos Monza — prefijo /api/monza/sii-libro/match.

Espejo DELIBERADO de wasabil_compras/router_match.py (GA): cero imports de
wasabil_compras/ (regla de la casa). Mismo estilo (y mismo candado, acá 'automotriz')
que el router hermano del libro Monza. Router PROPIO: el caller lo monta en main.py —
este módulo no toca main ni el router existente.
...
LO QUE PROTEGEN ESTOS ENDPOINTS (spec, reglas 2 y 3):
  ...
  · Confirmar enciende mov.conciliado: con eso el guard EXISTENTE de Tesorería Monza
...
ADAPTACIONES Monza (vs el molde GA): los modelos de banco/egresos NO tienen columna
`empresa` (separación POR TABLA) — se eliminan esos filtros; el candado de marca es
require_empresa("automotriz") a nivel de router.
"""
import json
import logging
...
from .models import (
    MonzaSiiLibroDoc, MonzaSiiLibroMatch, MonzaSiiMatchEtiquetaMov, MonzaSiiMatchRun,
)
...
router = APIRouter(
    prefix="/api/monza/sii-libro/match",
    tags=["monza-sii-libro-match"],
...
def _match_scoped(db: Session, match_id: int) -> MonzaSiiLibroMatch:
    m = (db.query(MonzaSiiLibroMatch)
         .filter(MonzaSiiLibroMatch.id == match_id).first())
    if not m:
        raise HTTPException(404, "Match no encontrado")
    return m


def _grupo_de(db: Session, m: MonzaSiiLibroMatch) -> list:
    ...
    if not m.grupo_uuid:
        return [m]
    return (db.query(MonzaSiiLibroMatch)
            .filter(MonzaSiiLibroMatch.grupo_uuid == m.grupo_uuid)
            .order_by(MonzaSiiLibroMatch.id).all())


def _mov_lock(db: Session, mov_id: int):
    """CALCA el patrón _mov_scoped lock=True de GA (OJO: el _mov_or_404 de
    monza_tesoreria no usa populate_existing — acá sí): LECTURA DE PLATA bajo lock →
    populate_existing obligatorio (el identity map serviría un monto viejo aunque el
    FOR UPDATE serialice)."""
...
def _solo_cuentas_clp(cuenta) -> None:
    """Mismo criterio y mensaje que monza_tesoreria/router.py (regla 21)."""
...
def _enlaces_internos(db: Session, mov_id: int) -> tuple:
    """ADAPTACIÓN Monza: monza_tes_conciliacion es XOR egreso/adelanto — las filas
    con adelanto_id también son 'enlace interno' (para el chequeo de estado a
    medias); en la tercera pata las salta el egreso None."""
    from monza_tesoreria.models import (MonzaTesConciliacion,
                                        MonzaTesConciliacionIngreso)
    egresos = db.query(MonzaTesConciliacion).filter(
        MonzaTesConciliacion.movimiento_id == mov_id).all()
    ingresos = db.query(MonzaTesConciliacionIngreso.id).filter(
        MonzaTesConciliacionIngreso.movimiento_id == mov_id).first()
    return egresos, ingresos


def _serializar(db: Session, filas: list) -> list:
    doc_ids = {m.doc_id for m in filas}
    docs = {d.id: d for d in db.query(MonzaSiiLibroDoc)
            .filter(MonzaSiiLibroDoc.id.in_(doc_ids)).all()} if doc_ids else {}
    mov_ids = [m.movimiento_id for m in filas if m.movimiento_id is not None]
    movs = {}
    if mov_ids:
        from monza_tesoreria.models import MonzaTesMovimiento
        movs = {mv.id: mv for mv in db.query(MonzaTesMovimiento)
                .filter(MonzaTesMovimiento.id.in_(mov_ids)).all()}
    out = []
    for m in filas:
        d = docs.get(m.doc_id)
...
def pendientes(
    ...
    if estado:
        ...
        q = q.filter(MonzaSiiLibroMatch.estado == estado)
    else:
        q = q.filter(MonzaSiiLibroMatch.estado.in_(ESTADOS_BANDEJA))
    if match_id is not None:
        q = q.filter(MonzaSiiLibroMatch.id == match_id)
    total = q.count()
    # score DESC: en MySQL los NULL (conflictos, sin score) quedan al final — el
    # orden del operador es «lo más seguro primero».
    filas = (q.order_by(MonzaSiiLibroMatch.score.desc(), MonzaSiiLibroMatch.id.desc())
...
def confirmar(match_id: int, db: Session = Depends(get_db),
    ...
    def _tx():
        ...
        for mid, mv in movs.items():
            ...
            confirmado = (db.query(func.coalesce(
                func.sum(MonzaSiiLibroMatch.monto_asignado), 0))
                .filter(MonzaSiiLibroMatch.movimiento_id == mid,
                        MonzaSiiLibroMatch.estado == "confirmado",
                        ~MonzaSiiLibroMatch.id.in_(ids_grupo)).scalar())
        ...
        # 409'. FOR UPDATE sobre las filas de monza_sii_libro_match de cada doc
        # (índice ix_monza_sii_match_doc_estado) — JAMÁS sobre la tabla del espejo
        # (regla 22 intacta: el barrido no toca la tabla de matches). Orden global
        # de locks: movimientos primero (arriba, por id ASC), docs por doc_id ASC.
        for did in sorted({g.doc_id for g in grupo}):
            (db.query(MonzaSiiLibroMatch.id)
             .filter(MonzaSiiLibroMatch.doc_id == did)
             .with_for_update().all())
        for g in grupo:
            doc = (db.query(MonzaSiiLibroDoc).filter(MonzaSiiLibroDoc.id == g.doc_id)
            ...
            confirmado_doc = (db.query(func.coalesce(
                func.sum(func.abs(MonzaSiiLibroMatch.monto_asignado)), 0))
                .filter(MonzaSiiLibroMatch.doc_id == g.doc_id,
                        MonzaSiiLibroMatch.estado == "confirmado",
                        ~MonzaSiiLibroMatch.id.in_(ids_grupo)).scalar())
            ...
            if egresos:
                ...
                for c in egresos:
                    if c.egreso_id is None:
                        continue     # enlace a adelanto (XOR Monza): no es tercera pata
                    e = db.query(MonzaContEgreso).filter(
                        MonzaContEgreso.id == c.egreso_id).first()
                    if e is None:
                        continue
                    ...
                    for det, compra in (db.query(MonzaContEgresoDetalle, MonzaContCompra)
                                        .join(MonzaContCompra,
                                              MonzaContCompra.id
                                              == MonzaContEgresoDetalle.compra_id)
                                        .filter(MonzaContEgresoDetalle.egreso_id == e.id)
                                        .all()):
    ...
    logger.info("Matcher Monza: match %s confirmado por usuario %s", match_id, uid)
    return resultado
...
def descartar(match_id: int, body: DescartarIn = None,
    ...
    if resultado is None:
        raise HTTPException(409, "Conflicto momentáneo con otra operación "
                                 "simultánea: reintente")
    logger.info("Matcher Monza: match %s descartado por usuario %s", match_id, uid)
    return resultado
...
def resumen(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    ...
    conteos = dict(db.query(MonzaSiiLibroMatch.estado,
                            func.count(MonzaSiiLibroMatch.id))
                   .group_by(MonzaSiiLibroMatch.estado).all())
    autos_vivos = (db.query(func.count(MonzaSiiLibroMatch.id))
                   .filter(MonzaSiiLibroMatch.estado == "confirmado",
                           MonzaSiiLibroMatch.origen == "auto",
                           MonzaSiiLibroMatch.divergente.is_(False)).scalar())
    ...
    total_libro = float(db.query(func.coalesce(
        func.sum(func.abs(MonzaSiiLibroDoc.monto_efectivo)), 0))
        .filter(MonzaSiiLibroDoc.estado_espejo == "ACTIVO").scalar() or 0)
    conciliado = float(db.query(func.coalesce(
        func.sum(func.abs(MonzaSiiLibroMatch.monto_asignado)), 0))
        .filter(MonzaSiiLibroMatch.estado == "confirmado").scalar() or 0)
    etiquetas = dict(db.query(MonzaSiiMatchEtiquetaMov.etiqueta,
                              func.count(MonzaSiiMatchEtiquetaMov.id))
                     .filter(MonzaSiiMatchEtiquetaMov.revertida.is_(False))
                     .group_by(MonzaSiiMatchEtiquetaMov.etiqueta).all())
    ...
    ultima = (db.query(MonzaSiiMatchRun)
              .filter(func.coalesce(MonzaSiiMatchRun.error, "") != ERROR_FENCING)
